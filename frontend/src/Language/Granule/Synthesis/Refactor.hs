...
bubbleUpPatterns gradedVars (App _ _ _ (Val _ _ _ (Abs _ p _ e)) (Val _ _ _ (Var _ x))) pats | x `notElem` gradedVars =
...
bubbleUpPatterns gradedVars (Case _ _ _ (Val _ _ _ (Var _ name)) [(p, expr)]) pats | name `notElem` freeVars expr = do
