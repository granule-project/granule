...
withPartialExpr = withPartialExprAt Just
...
isExpr = const False `extQ` isExpr'
  where
    isExpr' :: Expr () () -> Bool
    isExpr' _ = True
...
isValue = const False `extQ` isValue'
  where
    isValue' :: Value () () -> Bool
    isValue' _ = True
...
isExprFix2 = const False `extQ` isExprFix2'
  where
...
isValueFix2 = const False `extQ` isValueFix2'
  where
