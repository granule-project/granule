...
instance Pretty (Neg Constraint) where
    wlpretty (Neg (Neq _ c1 c2 _)) =
      "Trying to prove that " <> wlpretty c1 <> " == " <> wlpretty c2

    wlpretty (Neg (Eq _ c1 c2 _)) =
      "Actual grade or index `" <> wlpretty c1 <>
      "` is not equal to specified grade or index `" <> wlpretty c2 <> "`"

    wlpretty (Neg (ApproximatedBy _ c1 c2 (TyCon k))) | internalName k == "Level" =
      wlpretty c2 <> " value cannot be moved to level " <> wlpretty c1

    wlpretty (Neg (ApproximatedBy _ c1 c2 k)) =
      case k of
        TyCon (internalName -> "Nat") ->
          "Expected " <> wlpretty c2 <> " uses, but instead there are " <> wlpretty c1 <> " actual uses."
        _ -> wlpretty c1 <> " is not approximatable by " <> wlpretty c2 <> " for type " <> wlpretty k

    wlpretty (Neg p@Lub{}) =
      "Trying to prove negation of statement: " <> wlpretty p

    wlpretty (Neg (Lt _ c1 c2)) =
      "Trying to prove false statement: (" <> wlpretty c1 <> " < " <> wlpretty c2 <> ")"

    wlpretty (Neg (Gt _ c1 c2)) =
      "Trying to prove false statement: (" <> wlpretty c1 <> " > " <> wlpretty c2 <> ")"

    wlpretty (Neg (LtEq _ c1 c2)) =
      "Trying to prove false statement: (" <> wlpretty c1 <> " ≤ " <> wlpretty c2 <> ")"

    wlpretty (Neg (GtEq _ c1 c2)) =
      "Trying to prove false statement: (" <> wlpretty c1 <> " ≥ " <> wlpretty c2 <> ")"

    wlpretty (Neg (Hsup _ c1 c2 t)) =
      "Cannot split resource requirements into " <> wlpretty c1 <> " and " <> wlpretty c2 <>
      " in a pattern match. \nTrying to prove false statement: (" <> wlpretty c1 <> " ⨱ " <> wlpretty c2 <> ")"
...
instance Pretty Constraint where
    wlpretty (Eq _ c1 c2 _) =
      "(" <> wlpretty c1 <> " = " <> wlpretty c2 <> ")" -- @" <> show s

    wlpretty (Neq _ c1 c2 _) =
        "(" <> wlpretty c1 <> " ≠ " <> wlpretty c2 <> ")" -- @" <> show s

    wlpretty (ApproximatedBy _ c1 c2 k) =
      case k of
        -- Nat is discrete
        TyCon (internalName -> "Nat") ->
            "(" <> wlpretty c1 <> " = " <> wlpretty c2 <> ")"
        -- Sets (subset)
        ...
        _ -> "(" <> wlpretty c1 <> " ≤ " <> wlpretty c2 <> ")" -- <> " @ " <> pretty k

    wlpretty (Lub _ c1 c2 c3 _ _) =
      "(" <> wlpretty c1 <> " ⊔ " <> wlpretty c2 <> " = " <> wlpretty c3 <> ")"
    ...
    wlpretty (LtEq _ c1 c2) =
      "(" <> wlpretty c1 <> " ≤ " <> wlpretty c2 <> ")"

    wlpretty (GtEq _ c1 c2) =
      "(" <> wlpretty c1 <> " ≥ " <> wlpretty c2 <> ")"

    wlpretty (Hsup _ c1 c2 t) =
      "(" <> wlpretty c1 <> " ⨱ " <> wlpretty c2 <> ")"
...
instance Pretty Pred where
  wlpretty =
    ...
    where
      ...
      prettyBinding (id, k) = wlpretty id <> " : " <> wlpretty k
