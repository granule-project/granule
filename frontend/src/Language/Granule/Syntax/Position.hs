...
module Language.Granule.Syntax.Position
  ( -- * Positions
    Position
  , PositionWithoutFile
  , Position'(..)
...
instance Pretty a => Pretty (Position' (Maybe a)) where
  pretty (Pn Nothing  _ l c) = pretty l <> text "," <> pretty c
  pretty (Pn (Just f) _ l c) =
    pretty f <> colon <> pretty l <> text "," <> pretty c

instance Pretty PositionWithoutFile where
  pretty p = pretty (p { srcFile = Nothing } :: Position)

instance Pretty IntervalWithoutFile where
  pretty (Interval s e) = start <> text "-" <> end
    where
      sl = posLine s
      el = posLine e
      sc = posCol s
