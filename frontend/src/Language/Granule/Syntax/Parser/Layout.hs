...
-}
module Language.Granule.Syntax.Parser.Layout
...
import Language.Granule.Syntax.Parser.LexActions
...
{-| Executed for the first token in each line (see 'Language.Granule.Syntax.Parser.Lexer.bol').
    ...
    - /to the left/ :
        ...
        'Language.Granule.Syntax.Parser.Lexer.bol' state).

    - /same column/ :
        Exit the 'Language.Granule.Syntax.Parser.Lexer.bol' state and return a virtual semi
        colon.

    - /to the right/ :
        Exit the 'Language.Granule.Syntax.Parser.Lexer.bol' state and continue lexing.
...
{-| This action is only executed from the 'Language.Granule.Syntax.Parser.Lexer.empty_layout'
    state. It will exit this state, enter the 'Language.Granule.Syntax.Parser.Lexer.bol' state,
...
{-| Start a new layout context. This is one of two ways to get out of the
    'Language.Granule.Syntax.Parser.Lexer.layout' state (the other is 'openBrace'). There are
    two possibilities:
    ...
    block so we enter the 'Language.Granule.Syntax.Parser.Lexer.empty_layout' state. In both
