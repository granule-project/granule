...
data ExprF ev a expr value =
  ...
  | HandledF Span a Bool expr TypeScheme [(Pattern a, ([Pattern a], expr))]
...
pattern Handled sp a rf exp t oprs = (ExprFix2 (HandledF sp a rf exp t oprs))
...
instance Functor (Expr ev) where
  ...
  fmap f (Handled s a b expr ht os) = Handled s (f a) b (fmap f expr) ht (map (\(o, (ps,e)) -> (fmap f o, (map (\p -> fmap f p) ps, fmap f e))) os)
...
instance Rp.Refactorable (Expr ev a) where
  ...
  isRefactored (Handled _ _ True _ _ _) = Just Rp.Replace
...
instance Term (Expr v a) where
    ...
    freeVars (Handled _ _ _ e _ os) = ( ( freeVars e <> concatMap (\(o,(ps,e)) -> (concatMap freeVars ps)) os <> concatMap (\(o,(ps,e)) -> freeVars e) os )
                                    \\ concatMap (\(o,(ps,e)) -> (concatMap boundVars ps)) os )
                                    \\ concatMap (boundVars . fst) os
    ...
    hasHole (Handled _ _ _ e _ os) = hasHole e || (or (map (\(o,(ps,e)) -> (or (map (\p -> hasHole p) ps))) os)) || (or (map (\(o,(ps,e)) -> hasHole e) os)) 
...
instance Substitutable Expr where
    ...
    subst es v (Handled s a rf expr t oprs) = 
      Handled s a rf (subst es v expr) t
              (map (second (second (subst es v))) oprs)
...
instance Monad m => Freshenable m (Expr v a) where
    ...
    freshen (Handled s a rf expr t oprs) = do
      expr    <- freshen expr
      oprs <- forM oprs $ \(o, (ps,e)) -> do
                  o <- freshen o
                  ps <- forM ps $ \p -> freshen p
                  e <- freshen e
                  removeFreshenings (boundVars o)
                  return (o, (ps,e))
